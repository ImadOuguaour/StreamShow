...
export const fetchPopularMovies = (): AppThunk => async (dispatch) => {
  ...
  try {
    const response = await axios.get(`${TMDB_API_URL}/movie/popular?api_key=${API_KEY}&sort_by=popularity.desc`);
...
export const fetchPopularTv = (): AppThunk => async (dispatch) => {
  ...
  try {
    const response = await axios.get(`${TMDB_API_URL}/tv/popular?api_key=${API_KEY}&sort_by=popularity.desc`);
